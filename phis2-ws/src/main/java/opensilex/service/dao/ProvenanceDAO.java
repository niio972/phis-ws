...
public class ProvenanceDAO extends MongoDAO<Provenance> {
    ...
    private final static String DB_FIELD_EXPERIMENTS = "experiments";
     ...
     * @example { "metadata.SensingDevice" :
     * "http://www.opensilex.org/demo/s001", "metadata.Vector" :
     * "http://www.opensilex.org/demo/v001", "uri" :
     * "http://www.opensilex.org/opensilex/id/provenance/1551805521606", "label"
     * : { "$regex" : "PROV2019-LEAF", "$options" : "" }, "comment" : { "$regex"
     * : "plant", "$options" : "" } }
     * @return the query
     */
     ...
     *
     * @example { "uri" :
     * "http://www.opensilex.org/opensilex/id/provenance/1551877498746", "label"
     * : "PROV2019-LEAF", "comment" : "In this provenance we have count the
     * number of leaf per plant", "metadata" : { "SensingDevice" :
     * "http://www.opensilex.org/demo/s001", "Vector" :
     * "http://www.opensilex.org/demo/v001" } }
     * @param provenance
    ...
    private Document prepareInsertProvenanceDocument(Provenance provenance) {
        ...
        provenanceDocument.append(DB_FIELD_EXPERIMENTS, provenance.getExperiments());
    ...
    /**
     * Inserts the given provenances. No check is needed, the Java Beans
    ...
    public POSTResultsReturn checkAndInsert(List<Provenance> provenances) {
        POSTResultsReturn checkExperimentsResults = checkExperiments(provenances);
        if (!checkExperimentsResults.getDataState()) {
            return checkExperimentsResults;
        }
        try {
     ...
     */
    private POSTResultsReturn checkExperiments(List<Provenance> provenances) {
        POSTResultsReturn checkResult;
        List<Status> checkStatus = new ArrayList<>();
        ExperimentSQLDAO experimentRdf4jDAO = new ExperimentSQLDAO();
        boolean dataOk = true;
        for (Provenance provenance : provenances) {
            //Check if the provenance uri exist.
            for (String experiment : provenance.getExperiments()) {
                Experiment experiment1 = new Experiment(experiment);
                try {
                    if (!experimentRdf4jDAO.existInDB(experiment1)) {
                        dataOk = false;
                        checkStatus.add(new Status(StatusCodeMsg.WRONG_VALUE, StatusCodeMsg.ERR,
                                "The given experiment uri (" + experiment + ") does not exist"));
                    }
                } catch (Exception ex) {
                   return new POSTResultsReturn(false, Response.Status.INTERNAL_SERVER_ERROR, ex.toString());
                }
     ...
     * @param uri
     * @example {"uri":
     * "http://www.opensilex.org/demo/id/provenance/0193759540"}
     * @return true if the provenance exist, false if it does not exist.
     */
    ...
    /**
     * Updates the given provenances. /!\ Prerequisite : data must have been
     ...
     * @param searchProvenance
     * @param jsonValueFilter
     * @return the list of the provenances corresponding to the given search
     * parameters
     */
     ...
     *
     * @param label
     * @example {"label": {"$regex": "provenance", "$options": ""}}
